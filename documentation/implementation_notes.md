...
Mitigation is partial: Phase 5 uses a hybrid strategy that uses `inherit_scale='ALIGNED'` for uniform-scale chains and a direct SRT delta + `inherit_scale='NONE'` for non-uniform ones. Deeply-nested ornamental chains (tentacles, feelers, appendages with near-zero rest scale) are the remaining failure case.
