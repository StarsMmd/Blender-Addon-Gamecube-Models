"""Blender material-anim fcurves → IRMaterialTrack decoder.
...
Used as the deep-work backend for animations decoding. Future work can
shape these tracks into ``BRMaterialTrack`` and move the conversion
into ``plan/helpers/animations.py``.
...
The ``material_mesh_name`` identifier format is
``"mesh_{padded_idx}_{bone_name}"`` — the *first* IRMesh (in
describe_meshes iteration order) that uses each material, matching how
the importer keyed its ``material_lookup`` dict.
"""
import re
