...
class ModelSet(Node):
    ...
    def _createArmature(self, builder):
        ...
        self.addConstraints(builder, armature, bones)
    ...
    def addConstraints(self, builder, armature, bones):
        for hsd_joint in bones:
            if hsd_joint.flags & JOBJ_TYPE_MASK == JOBJ_EFFECTOR:
                if not hsd_joint.temp_parent:
                    if builder.options.get("verbose"):
                        print("IK Effector has no Parent:", hsd_joint.temp_name)
                    continue
                ...
                poletarget_robj = hsd_joint.getReferenceObject(Joint, 0)
                length_robj = hsd_joint.getReferenceObject(BoneReference, 0)

                if not length_robj:
                    if builder.options.get("verbose"):
                        print("No Pole angle and bone length constraint on IK Effector Parent:", hsd_joint.temp_name)
                    continue
