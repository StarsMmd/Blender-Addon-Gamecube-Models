...
class Image(Node):
    ...
    def decodeFromRawData(self, palette):
        ...
        Uses self.raw_image_data stored during loadFromBinary(). Delegates to
        the shared decode_texture codec, which handles tile iteration, crop,
        vertical flip, and the vectorized CMPR fast path.
        Returns cropped, vertically-flipped RGBA byte array, or None if no data.
        """
        if not self.raw_image_data:
            return None
        return decode_texture(self.raw_image_data, self.width, self.height,
                              self.format, palette)
