...
- **Envelopes — walled (grouped, not internally ordered).** `EnvelopeList`s vary in size (16 or 24 bytes) so order matters, and `PObject.property` is a `(*EnvelopeList)[]` array. No traversal tried (joint-tree pre/post, PObject DFS/address order, array forward/reverse, first/last dedup, all ×reverse — three sweep rounds) scored above **16/462**. Until the convention is found, envelopes are **grouped at their phase position in node_list order** — internally still mis-ordered, but grouping keeps the downstream phases contiguous so they stay correctly ordered.
...
- **Palettes — cracked (pikachu/eievui), via `_MATERIAL_TYPES`.** A texture's `Palette` struct belongs to the material block (it sits `Image, Palette, Texture` in the DFS). `Palette` was missing from `_MATERIAL_TYPES`, so the block-bucketing scattered it into the trailing `rest` region instead of carrying it with its texture under the reverse-joint-post block order. Adding `Palette` to the set groups it correctly.

**Metric — ordering correctness, not BNB.** Convergence is judged by `tools/bnb_node_ordinals.py --lis`: a node is *misplaced* if it isn't in the longest run of monotonically-increasing original offsets through our export order (N − LIS). This is robust to absolute region shifts (a region that is internally well-ordered but shifted as a whole stays in the LIS), unlike global BNB or a high-water backward-step count — both of which a single early shift makes look catastrophic. With materials, vertices, texture-animations, multi-texture chains, palettes and the scene tail integrated, **every non-envelope struct is in correct relative order on all five test models** (absol/metamon/pikachu/deoxys/eievui — all remaining misplacements are `EnvelopeList`). Envelopes are the sole remaining unordered region (deliberately walled — see above). Note global BNB *temporarily* drops on near-byte-exact models (e.g. metamon) while phases are partially integrated, because the not-yet-byte-exact envelope grouping shifts the file — this is expected and ignored; BNB only peaks once every phase (envelopes included) is byte-exact.
