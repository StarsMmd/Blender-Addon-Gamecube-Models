...
### Animation quantization selection (corpus-derived)
The FObj `frac_value`/`frac_slope` byte packs a storage type (top 3 bits: FLOAT/S16/U16/S8/U8) and fraction-bit count (low 5). A full-corpus analysis (`tools/analyze_frame_encoding.py`, 159,499 Frame streams across 87 models) established the original compiler's behavior:

- **Fraction formula is universal**: `frac = usable_bits − ceil(log2(max_abs + 1))` with usable bits U8 8, S8 7, U16 16, S16 15. It matches ~100% of original bytes per type.
- **Type selection is smallest-bit-exact**: the smallest type (1-byte before 2-byte, unsigned before signed) whose formula frac represents *every* value exactly. On round-tripped data the decoded stream already sits on its source grid, so this recovers the original choice — 88.1% of value bytes and 91.1% of slope bytes corpus-wide with a FLOAT fallback.
- **The residue is unrecoverable**: all-zero channels' original byte encodes the *pre-quantization* raw residue (raw max exactly 0 → U8.8; tiny-but-nonzero raw max ≤ 1 → U8.7 or S8.6), information destroyed by the original quantization. Plus ~400 signed-choice oddities and channels stored as FLOAT.

`_pick_quantization` (compose) implements the exact pass first, then falls back to the previous tolerance-based smallest-fit for values with no exact representation (fresh Blender-authored animations) — deliberately *not* the FLOAT fallback, which would double per-keyframe cost on fresh exports; that costs ~2.4 value / ~10.3 slope points of round-trip byte fidelity relative to the measured ceiling and can be revisited if the size budget allows.

### Animation channel order (Frame chains)
Game-native Animation Frame chains order joint channels **scale, rotation, translation** (X,Y,Z ascending within each group). Corpus counts: with scale present, `S→R→T` patterns outnumber `R…S` ones ~8,000 : 81; without scale, `R→T` dominates `T→R` 5,576 : 217; within-group ascending holds 95%. Compose emits channels in that order (`_build_animation`). Before this landed, compose emitted `R,T,S` — every scale-bearing chain misaligned, and because the NIN comparator pairs frames positionally along `.next`, one misordered chain cascaded into type/frac/ad/length errors for every downstream channel; this alone was ~20 NIN points on scale-animated models. The known-imperfect minority: `TR`/`RS`/`ST` chains (~330 corpus-wide) and PATH-channel placement variants (`SPTR`/`RPTS`/`PTR`, ~85).

Compose's opcode emission mirrors the decoder's slope state machine exactly: spline keys with zero outgoing tangent encode as `SPL0` (value-only), and an `SLP` node is emitted wherever a key's incoming tangent differs from the running slope — **including mid-stream**, where two adjacent spline keys have discontinuous tangents. Before this landed the encoder only handled the run-start case and silently dropped mid-run tangent overrides, changing the interpolated motion of asymmetric-tangent animations on re-export (the SLP-interleaved streams the Keyframe-interpolation note above assumed Colo/XD never emits do in fact occur in Pokémon files). Byte-identical stream reproduction (Part A of the analysis tool) sits at 65.5% of all Frame streams; the remaining divergence is ~29% frac-byte (slope-FLOAT originals + zero-channel residue above) and ~5% stream-length (residual run-splitting/`KEY`/`NONE` opcode nuances).
