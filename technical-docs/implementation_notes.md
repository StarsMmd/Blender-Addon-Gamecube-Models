...
Remaining Frame-encoding divergence after both fixes (Part A of the analysis tool): ~29% of streams still differ in the frac byte (slope-FLOAT originals + zero-channel residue above), and ~14% differ only in stream length — the original encoder emits `SPL0` (value-only spline keys, tangent carried forward) where ours always emits `SPL` (value+slope), plus run-splitting differences. That opcode-level packing is the next lever.
