...
A `JOBJ_EFFECTOR` joint's stored translation is the IK *target reach*, not its bind-pose bone length: the direction is meaningful but the magnitude is large (the reach the solver drives toward). Describe's naive SRT accumulation treated it as an ordinary joint offset, so the effector — and every descendant joint plus every vertex skinned to them — landed tens of units off-model. The visible symptom on the Colosseum maps was palm-tree crowns/fronds flung far past their trunks (`M1_out`: 3 effectors, translation magnitude ~15× the true length; `D6_out_all`: 24 effectors, ~100×). The stored inverse-bind matrices for the crown joints encode the correct compact pose, but they're model-root-relative (near-origin), so they can't be used directly as world matrices — the trunk joints *above* the effector already get the right world from SRT; only joints *below* the effector inherit the error.

The true bind length lives in the `BoneReference` (IK hint, offset `0x0`) on the effector's **parent** joint — the same length the pole-flip path and the importer's edit-bone reposition already read. **Fix** (`importer/phases/describe/helpers/bones.py::_effector_bind_position`): when describing a `JOBJ_EFFECTOR`, rescale its (metres) translation to that `BoneReference` length, preserving direction, before composing its world matrix. World matrices cascade to descendants and feed the envelope deform in the same phase, so the whole crown collapses back onto the trunk and the skinned mesh follows. It's a no-op wherever `|translation| == BoneReference.length` (a well-formed chain), so it only touches genuinely stretched effectors — which in the corpus are map foliage only (no game-native PKX/Pokémon model carries an IK effector). Round-trip is unaffected: the exporter already writes the reposition length (not the raw target), so the raw ~reach magnitude was never preserved.
