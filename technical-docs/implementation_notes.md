...
The exporter recomputes AABBs from scratch (linear-blend skinning of every mesh vertex at every frame), so they're a derived quantity, not round-tripped — `BNB` reuses the parsed bytes (faithful), `NIN`/real exports recompute. With frames correctly aligned, the residual recompute error is dominated by a **systematic over-extension of `max.y`** (the +Y/up extent): consistently positive and ≈1.4× the model's true top extent (e.g. absol +4.3, nukenin +5.2 units, every frame), while the other five components scatter within ~1 unit. The signature (always-positive, top-only, scales with model height) points to our skinned vertex set reaching higher than the game's box — likely a vertex-subset or per-bone evaluation difference at the top of the rig, not a coordinate or format issue. Open for follow-up.
