...
Our shader-node simulation is more aggressive than the in-game TEV (it multiplies after vertex-colour shading), so we skip insertion on materials whose colour chain has no texture. Phase 6, the standalone `add_shiny_filter` script, and the `prepare_for_pkx_export` script all detect this by walking back from the shader's color input: if no `ShaderNodeTexImage` is reachable from that input (including the unlinked / default-valued case), the filter is not inserted. Materials with a real `ShaderNodeTexImage` anywhere in the chain — regardless of whether the colour source is `MATERIAL`, `VERTEX`, or `BOTH` — still get the filter.
