...
> **Scope of its optimisations:** prep applies the optimisations needed to land within the game's **hard** limits — ≤ 3 bone weights per vertex, 10 % weight quantisation — plus an aggressive texture-size cap (`MAX_TEXTURE_DIM`, default **64** px on the longer axis) because battle-model file size is dominated by texture pixels. Raise `MAX_TEXTURE_DIM` per-model if a texture looks too soft. If the exported file is still too large, or the target is a heavier newer-game rip, reach for `optimize.py` (or the individual `optimize/*.py` passes) — those are lossier but more aggressive (polycount decimation, keyframe thinning, vertex merging).
